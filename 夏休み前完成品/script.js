...
function searchBooks() {
  ...
  fetch(google_api_key)
    ...
    .then(data => {
      ...
      ////二番目のデータ(data1)についてデータの取得が完了するまで繰り返す////
      const fetchData1 = () => {
        //つくば市立図書館の蔵書検索
        fetch(api_key1_id_1)
          .then(response => response.json())
          .then(data => {
            if (data.continue === 1) {
              fetchData1(); // 再帰的にリクエストを行う
            } else {
              for (const key in data.books) {
                const lib_key_data1_id_1 = data.books[key][system_id_1]["libkey"];
                var lib_key_data1_str_id_1 = JSON.stringify(lib_key_data1_id_1); //data0のlibkey（貸出状況）をjson形式→文字列
                // const lib_key_data1_str_id_1_n_1 = Object.entries(lib_key_data0_str_id_1)[0];
                // console.log(lib_key_data1_str_id_1_n_1);
                console.log(lib_key_data1_str_id_1[0]);
                console.log(lib_key_data1_id_1);
                console.log(lib_key_data1_str_id_1);
                document.getElementById("title_data1").innerHTML = "タイトル:" + title_data1; //data0のタイトルをHTMLに表示
                // 「一冊目」とHTMLに表示
                $('#data1>h1').html("一冊目");
                //本の著者を表示
                $("#author_area_data1").html("著者:" + author_data1);
                // 本の表紙をHTMLに表示
                var img_scr = "<img src='" + thumbnail_data1 + "'/>";
                document.getElementById("image_area_data1").innerHTML = img_scr;
                // 貸出状況をHTMLに表示
                document.getElementById("lending_status_data1_id_1").innerHTML = "<h3>蔵書状況</h3>つくば市立中央図書館<br>" + lib_key_data1_str_id_1;
              }
              // continueが0になった時の処理  data0のlibkey（貸出状況）を取得
            }
          })
        ...
        .then(data => {
          if (data.continue === 1) {
            ...
            for (const key in data.books) {
              const lib_key_data1_id_2 = data.books[key][system_id_2]["libkey"];
              var lib_key_data1_str_id_2 = JSON.stringify(lib_key_data1_id_2); //data0のlibkey（貸出状況）をjson形式→文字列
              document.getElementById("lending_status_data1_id_2").innerHTML = "筑波大学附属中央図書館<br>" + lib_key_data1_str_id_2;
              fetchData2();
            }
            // continueが0になった時の処理  data1のlibkey（貸出状況）を取得
          }
        })
        .catch(error => {
          console.log('エラーが発生しました', error);
      ...
      ////三番目のデータ(data1)についてデータの取得が完了するまで繰り返す////
      const fetchData2 = () => {
        //つくば市立図書館の蔵書検索
        fetch(api_key2_id_1)
          .then(response => response.json())
          .then(data => {
            if (data.continue === 1) {
              fetchData2(); // 再帰的にリクエストを行う
            } else {
              for (const key in data.books) {
                const lib_key_data2_id_1 = data.books[key][system_id_1]["libkey"];
                var lib_key_data2_str_id_1 = JSON.stringify(lib_key_data2_id_1); //data0のlibkey（貸出状況）をjson形式→文字列
                // const lib_key_data1_str_id_1_n_1 = Object.entries(lib_key_data0_str_id_1)[0];
                // console.log(lib_key_data1_str_id_1_n_1);
                console.log(lib_key_data2_str_id_1[0]);
                console.log(lib_key_data2_id_1);
                console.log(lib_key_data2_str_id_1);
                document.getElementById("title_data2").innerHTML = "タイトル:" + title_data2; //data0のタイトルをHTMLに表示
                // 「一冊目」とHTMLに表示
                $('#data2>h1').html("一冊目");
                //本の著者を表示
                $("#author_area_data2").html("著者:" + author_data2);
                // 本の表紙をHTMLに表示
                var img_scr = "<img src='" + thumbnail_data2 + "'/>";
                document.getElementById("image_area_data2").innerHTML = img_scr;
                // 貸出状況をHTMLに表示
                document.getElementById("lending_status_data2_id_1").innerHTML = "<h3>蔵書状況</h3>つくば市立中央図書館<br>" + lib_key_data2_str_id_1;
              }
              // continueが0になった時の処理  data2のlibkey（貸出状況）を取得
            }
          })
          .catch(error => {
            console.log('エラーが発生しました', error);
        ...
        .then(data => {
          if (data.continue === 1) {
            ...
            for (const key in data.books) {
              const lib_key_data2_id_2 = data.books[key][system_id_2]["libkey"];
              var lib_key_data2_str_id_2 = JSON.stringify(lib_key_data2_id_2); //data0のlibkey（貸出状況）をjson形式→文字列
              document.getElementById("lending_status_data2_id_2").innerHTML = "筑波大学附属中央図書館<br>" + lib_key_data2_str_id_2;
            }
            // continueが0になった時の処理  data2のlibkey（貸出状況）を取得
          }
        })
